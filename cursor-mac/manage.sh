#!/bin/bash
# Cursor MCP Bridge Manager (macOS)
...
show_menu() {
    clear
    echo "========================================"
    echo "  Cursor MCP Bridge Manager (Mac)"
    echo "========================================"
    echo ""
    echo "  Ports:"
...
start_all() {
    ...
    cd "$SCRIPT_DIR"
    pm2 delete cursor-mcp-bridge 2>/dev/null || true
    pm2 start ecosystem.config.cjs
    echo ""
    echo "[2/2] Database MCP..."
    cd "$SCRIPT_DIR/mcp-database"
    pm2 delete cursor-mcp-database 2>/dev/null || true
    pm2 start ecosystem.config.cjs
    echo ""
    echo "========================================"
    ...
    curl -s http://localhost:3211/health 2>/dev/null || echo "(not responding)"
    echo ""
    echo ""
    echo "[Database MCP - Port 3212]"
    curl -s http://localhost:3212/health 2>/dev/null || echo "(not responding)"
    echo ""
    echo ""
...
stop_all() {
    ...
    echo "========================================"
    echo ""
    pm2 stop cursor-mcp-bridge 2>/dev/null || true
    pm2 delete cursor-mcp-bridge 2>/dev/null || true
    echo "[OK] cursor-mcp-bridge killed."
    pm2 stop cursor-mcp-database 2>/dev/null || true
    pm2 delete cursor-mcp-database 2>/dev/null || true
...
logs_browser() {
    ...
    echo "========================================"
    echo ""
    pm2 logs cursor-mcp-bridge --lines 50 --nostream 2>/dev/null
    echo ""
    read -n 1 -s -r -p "Press any key to continue..."
}
...
logs_database() {
    ...
    echo "========================================"
    echo ""
    pm2 logs cursor-mcp-database --lines 50 --nostream 2>/dev/null
    echo ""
    read -n 1 -s -r -p "Press any key to continue..."
}
